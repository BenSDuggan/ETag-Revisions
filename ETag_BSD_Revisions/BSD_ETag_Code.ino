...
bool sleep = false; //If the board experienced a sleep change of starte hold it here
...
void loop() {  //This is the main function. It loops (repeats) forever.
  // Run if awake and turn off the antneas if not
  if(awake()) {
...
void clockSleepSetup(){
  attachInterrupt(digitalPinToInterrupt(7), EIC_ISR, LOW);  // Attach interrupt to pin 7 with an ISR and when the pin state CHANGEs
