...
/*** START - Delayed Sleep ***/
bool delayByTime = true; //If you want to delay the start of the unit by number of days, hours, minutes and seconds set this to true and delayByStartDate to false
//bool delayByStartDate = false; //If you want to delay the start of the unit to start on a given month, day, year, hour, minute and second set this to true and delayByTime to false
...
void goToSleep() {
  ...
  */
  delay(20); //Wait 118 seconds and then sleep
  // Flash the LED 5 times
  /*
  for (int i = 0; i < 5; i++) {
    digitalWrite(LED_RFID, LOW);
...
void logRFID_To_SD(EM4100Data *xd) {
  ...
  if (dataFile) {
    ...
    serial.println("error opening " + readerID + "data.txt"); //error message if the "datafile.txt" is not present or cannot be created  }// end check for file
  }// end check for file
}
